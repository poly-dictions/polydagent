...
class XOAuthAPI:
    ...
    @staticmethod
    async def get_mentions(access_token: str, user_id: str, since_id: str = None) -> List[Dict]:
        """Get mentions for a user using OAuth access token"""
        try:
            params = {
                "max_results": 20,
    ...
    async def post_tweet(access_token: str, text: str, reply_to: str = None) -> Optional[str]:
        ...
        # Truncate to 280 chars for non-Blue accounts
        if len(text) > 280:
            text = text[:277] + "..."
        payload = {"text": text}
        if reply_to:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 201:
                        ...
                        logger.error(f"X OAuth 401 Unauthorized - token may need refresh: {result}")
                        return "NEEDS_REFRESH"  # Signal to caller to refresh token
                    else:
                        logger.error(f"X OAuth tweet error (status {resp.status}): {result}")
                        # Log detailed error info
                        errors = result.get("errors", [])
                        for err in errors:
                            logger.error(f"  X API Error: {err.get('message', 'Unknown error')} (code: {err.get('code')})")
                        return None
        except Exception as e:
    ...
    async def post_thread(access_token: str, tweets: List[str]) -> Optional[str]:
        ...
        if first_tweet_id == "NEEDS_REFRESH":
            logger.error("X OAuth token needs refresh - cannot post thread")
        ...
        for i, tweet_text in enumerate(tweets[1:], start=2):
            ...
            if reply_id and reply_id != "NEEDS_REFRESH":
...
class AIAnalyzer:
    ...
    async def analyze_market(title: str, yes_odds: float, no_odds: float, volume: float, custom_prompt: str = "", niche: str = "general", is_multi: bool = False, top_options: list = None) -> Optional[Dict]:
        ...
        if not OPENROUTER_API_KEY and not FACTSAI_API_KEY:
            logger.warning("No API keys for market analysis (OPENROUTER_API_KEY or FACTSAI_API_KEY)")
            return None
        ...
        # Use Xiaomi via OpenRouter for structured analysis (free)
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        try:
                            ...
                            logger.info(f"AI analysis (Xiaomi): signal={analysis.get('signal')}, confidence={analysis.get('confidence')}")
                            return analysis
                        except json.JSONDecodeError as e:
                            logger.warning(f"JSON parse error: {e}, raw: {text[:500]}")
                            # Try to extract signal from text
                            text_lower = text.lower()
                            yes_pos = text_lower.find('"signal": "yes"') if '"signal": "yes"' in text_lower else 999
                            no_pos = text_lower.find('"signal": "no"') if '"signal": "no"' in text_lower else 999
                            signal = "yes" if yes_pos < no_pos else "no"
                            # Extract reasons using regex
                            import re
                            reasons = re.findall(r'"([^"]{15,60})"', text)[:3]
                            if not reasons:
                                reasons = ["AI provided unstructured analysis"]
                            return {
                                "signal": signal,
                                "confidence": "low",
                                "reasons": reasons,
                                "main_risk": "analysis format uncertain",
                                "notes": ""
                            }
                    else:
                        error = await resp.text()
                        logger.error(f"Xiaomi/OpenRouter API error: {resp.status} - {error[:200]}")
        except Exception as e:
...
async def process_agent_posting(agent_id: str, agent: Dict, scanner: PolymarketScanner, server: 'PolydictionsServer' = None) -> bool:
    ...
    if not analysis:
        # Don't post without proper AI analysis
        logger.warning(f"[{agent_id}] AI analysis failed, skipping post")
        return None
    ...
    if use_oauth and server:
        # Use X OAuth API
        logger.info(f"[{agent_id}] Using OAuth for posting, getting valid token...")
        valid_token = await server.get_valid_x_token(agent)
        if valid_token:
            logger.info(f"[{agent_id}] Got valid token, posting thread...")
            tweet_id = await XOAuthAPI.post_thread(valid_token, thread_tweets)
            logger.info(f"[{agent_id}] Post result: tweet_id={tweet_id}")

            # If token needs refresh, try to refresh and retry once
            if tweet_id == "NEEDS_REFRESH":
                logger.info(f"[{agent_id}] Token needs refresh, attempting refresh...")
                refresh_token = agent.get("x_refresh_token")
                if refresh_token:
                    new_tokens = await server.refresh_x_token(refresh_token)
                    if new_tokens:
                        ...
                        if agent_id in running_agents:
                            ...
                            save_agents()
                        logger.info(f"[{agent_id}] Token refreshed, retrying post...")
                        tweet_id = await XOAuthAPI.post_thread(new_tokens["access_token"], thread_tweets)
                        if tweet_id == "NEEDS_REFRESH":
                            tweet_id = None  # Refresh didn't help
                    else:
                        logger.error(f"[{agent_id}] Token refresh failed")
                        tweet_id = None
        else:
    ...
    if tweet_id and tweet_id != "NEEDS_REFRESH":
...
async def process_agent_mentions(agent_id: str, agent: Dict, server: 'PolydictionsServer' = None) -> int:
    ...
    use_oauth = bool(x_access_token and x_refresh_token and x_user_id)
...
async def start_agent(launch_id: str, launch: Dict[str, Any], credentials: Dict[str, str] = None, oauth_tokens: Dict[str, Any] = None):
    ...
    if oauth_tokens:
        # Support both expires_at (new) and expires_in (legacy)
        if oauth_tokens.get("expires_at"):
            expires_at = oauth_tokens.get("expires_at")
        else:
            expires_at = time.time() + oauth_tokens.get("expires_in", 7200)
        running_agents[agent_id].update({
        ...
        logger.info(f"Agent {agent_id} using X OAuth tokens (expires at {expires_at})")
    save_agents()
...
class APIServer:
    def __init__(self, host: str = "0.0.0.0", port: int = None):
        ...
        # Cache for market research/context (1 hour TTL) - saves FactsAI credits
        self.research_cache = TTLCache(ttl_seconds=3600, max_size=500)
    ...
    async def cors_middleware(self, request, handler):
        ...
        if request.method == 'OPTIONS':
            ...
            try:
                ...
                # HTTPException already has a response, add CORS headers to it
                response = e
            except Exception as e:
                # Other exceptions - create 500 response with error details
    ...
    async def get_events(self, request):
        ...
        # If platform=kalshi, redirect to dome events
        platform = request.query.get('platform', 'polymarket').lower()
        if platform == 'kalshi':
            return await self.get_dome_events(request)
    ...
    async def get_market_context(self, request):
        ...
        # Block updown markets - they change every 5-15 mins, research is useless
        combined = (slug + title).lower()
        if 'updown' in combined or 'up/down' in combined:
            return web.json_response({
                "success": False,
                "error": "AI analysis not available for Up/Down markets (prices change too quickly)"
            }, status=400)

        # If we have slug but no title, try to get title from the slug
        search_term = title if title else slug.replace('-', ' ')

        # Check cache first (saves FactsAI credits)
        cache_key = f"polymarket:{slug or search_term}"
        cached = self.research_cache.get(cache_key)
        if cached:
            logger.info(f"[Polymarket] Cache hit for: {cache_key[:50]}")
            return web.json_response({"success": True, "context": cached, "source": "cache"})
...
- Total length: 150-250 words"""
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if context:
                                ...
                                self.research_cache.set(cache_key, context)
                                return web.json_response({
                                    "success": True,
                                    "context": context,
                ...
                if facts_research:
                    ...
                    if len(facts_research) > 800:
                        clean_research += "..."
                    # Save to cache
                    self.research_cache.set(cache_key, clean_research)
                    return web.json_response({
                        "success": True,
    ...
    async def get_kalshi_context(self, request):
        ...
        # Check cache first (saves FactsAI credits)
        cache_key = f"kalshi:{ticker or title}"
        cached = self.research_cache.get(cache_key)
        if cached:
            logger.info(f"[Kalshi] Cache hit for: {cache_key[:50]}")
            return web.json_response({"success": True, "context": cached, "source": "cache"})
...
- Total length: 150-250 words"""
        ...
        try:
            ...
            async for field in reader:
                if field.name == 'token_image':
                    ...
                    if field.name == 'dev_buy_sol':
                        logger.info(f"📥 Received dev_buy_sol field: '{value}'")
            ...
            # Check if Twitter account already has a running agent
            twitter_username = fields['twitter_username'].replace('@', '').lower()
            for agent_id, agent in running_agents.items():
                if agent.get("twitter_username", "").lower() == twitter_username and agent.get("status") == "running":
                    return web.json_response({
                        "success": False,
                        "message": f"@{twitter_username} already has a running agent. One account = one agent."
                    }, status=400)
            ...
            raw_dev_buy = fields.get('dev_buy_sol', 0)
            logger.info(f"Raw dev_buy_sol from form: '{raw_dev_buy}' (type: {type(raw_dev_buy)})")
            dev_buy_sol = min(float(raw_dev_buy or 0), 85.0)
            logger.info(f"Parsed dev_buy_sol: {dev_buy_sol}")
            ...
            # Add OAuth tokens if provided
            if has_oauth:
                # x_expires_in is now absolute timestamp from frontend
                expires_at = int(fields.get('x_expires_in', time.time() + 7200))
                launch_data["x_oauth"] = {
                ...
                logger.info(f"Launch {launch_id} using X OAuth for @{fields['twitter_username']} (expires at {expires_at})")
            else:
                # Legacy credentials
            ...
            # Total payment: 0.05 fee + dev_buy amount
            total_payment = LAUNCH_FEE_SOL + dev_buy_sol
    ...
    async def launchpad_confirm(self, request):
        ...
        # Check if dev wallet has received payment (with retries)
        min_required = 0.02
        balance_sol = 0
        ...
        # Dev buy = what user requested (already paid for it)
        actual_dev_buy = launch.get("dev_buy_sol", 0)
        ...
        if result:
            ...
            # SOL remains in dev wallet - we have the private keys saved
    ...
    async def admin_wallets(self, request):
        ...
        # Check if searching for specific wallet
        search_wallet = request.query.get('wallet', '')

        wallets = []
        for lid, launch in pending_launches.items():
            dev_pub = launch.get("dev_wallet_public")

            # If searching, filter by wallet
            if search_wallet and dev_pub != search_wallet:
                continue
    ...
    async def admin_clear_agents(self, request):
        ...
        # Clear agents
        agents_count = len(running_agents)
        running_agents.clear()
        save_agents()

        # Clear launches
        launches_count = len(pending_launches)
        pending_launches.clear()
        save_launches()

        # Clear answered mentions and posted events
        answered = {}
        save_answered_mentions(answered)

        posted = {}
        save_posted_events(posted)

        logger.info(f"Admin cleared {agents_count} agents and {launches_count} launches")
    ...
    async def launchpad_agent_trigger_post(self, request):
        ...
        # Check for OAuth token or cookie
        has_oauth = agent.get("x_access_token") and time.time() < agent.get("x_token_expires_at", 0)
        has_cookie = agent.get("twitter_cookie")

        if not has_oauth and not has_cookie:
            return web.json_response({"success": False, "message": "No valid credentials (OAuth expired or no cookie)"}, status=400)
    ...
    async def launchpad_agent_test_oauth(self, request):
        ...
        debug_info = {
            "agent_id": agent_id,
            "twitter_username": agent.get("twitter_username"),
            "has_access_token": bool(agent.get("x_access_token")),
            "has_refresh_token": bool(agent.get("x_refresh_token")),
            "token_expires_at": agent.get("x_token_expires_at"),
            "token_expired": time.time() > agent.get("x_token_expires_at", 0),
            "current_time": time.time(),
        }

        # Try to get valid token
        valid_token = await self.get_valid_x_token(agent)
        debug_info["got_valid_token"] = bool(valid_token)

        if not valid_token:
            return web.json_response({"success": False, "debug": debug_info, "message": "Failed to get valid token"}, status=400)

        # Try to post a test tweet
        test_text = f"Test post from Polydictions agent - {int(time.time())}"
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    "https://api.twitter.com/2/tweets",
                    ...
                    result = await resp.json()
                    debug_info["x_api_status"] = resp.status
                    debug_info["x_api_response"] = result

                    if resp.status == 201:
                        tweet_id = result.get("data", {}).get("id")
                        return web.json_response({
                            "success": True,
                            "message": f"Test tweet posted: {tweet_id}",
                            "tweet_id": tweet_id,
                            "debug": debug_info
                        })
                    elif resp.status == 401:
                        # Try to refresh token
                        debug_info["trying_refresh"] = True
                        refresh_token = agent.get("x_refresh_token")
                        if refresh_token:
                            ...
                            if new_tokens:
                                ...
                                async with session.post(
                                    ...
                                    debug_info["retry_status"] = retry_resp.status
                                    debug_info["retry_response"] = retry_result
                                    if retry_resp.status == 201:
                                        tweet_id = retry_result.get("data", {}).get("id")
                                        return web.json_response({
                                            "success": True,
                                            "message": f"Test tweet posted after refresh: {tweet_id}",
                                            "tweet_id": tweet_id,
                                            "debug": debug_info
                                        })
                            else:
                        ...
                        return web.json_response({
                            "success": False,
                            "message": "X API 401 - token invalid even after refresh",
                            "debug": debug_info
                        }, status=401)
                    else:
                        return web.json_response({
            ...
            return web.json_response({"success": False, "debug": debug_info, "message": str(e)}, status=500)

    async def launchpad_agent_reauth(self, request):
        """Get re-authentication URL for an agent with invalid OAuth"""
        agent_id = request.match_info.get('agent_id')
        ...
        # Store for callback - use agent_id instead of launch_id for re-auth
        X_OAUTH_STATES[state] = {
            "code_verifier": code_verifier,
            "created_at": time.time(),
            "launch_id": agent_id,  # Use agent_id for existing agents
            "is_reauth": True
        }
    ...
    async def x_oauth_callback(self, request):
        ...
        try:
            ...
            # If this is a re-auth for an existing agent, update the agent directly
            is_reauth = state_data.get("is_reauth", False)
            if is_reauth and launch_id in running_agents:
                running_agents[launch_id]["x_access_token"] = access_token
                running_agents[launch_id]["x_refresh_token"] = refresh_token
                running_agents[launch_id]["x_token_expires_at"] = time.time() + expires_in
                running_agents[launch_id]["x_user_id"] = user_id
                save_agents()
                logger.info(f"Re-authenticated agent {launch_id} with @{username}")
    ...
    async def research_get_event(self, request):
        ...
        try:
            ...
            # Block updown markets - they waste FactsAI credits
            if 'updown' in slug.lower() or 'up/down' in slug.lower() or '-5m-' in slug.lower() or '-1h-' in slug.lower():
                return web.json_response({
                    "error": "AI analysis not available for Up/Down markets (prices change too quickly)"
                }, status=400)

            # Check cache first (saves FactsAI credits)
            cache_key = f"research:polymarket:{slug}"
            cached = self.research_cache.get(cache_key)
            if cached:
                logger.info(f"[Polymarket Research] Cache hit for: {slug[:50]}")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                # DISABLED: FactsAI for research page - prevents credit drain
                # FactsAI only used for: agents, explicit "get context" button clicks
                facts_research = ""
...
Use lowercase. Be concise and actionable. No markdown headers."""
                ...
                self.research_cache.set(cache_key, response_data)
    ...
    async def research_get_kalshi(self, request):
        ...
        try:
            ...
            # Block updown markets - they waste FactsAI credits
            ticker_lower_check = ticker.lower()
            if 'updown' in ticker_lower_check or 'up/down' in ticker_lower_check or '-5m-' in ticker_lower_check or '-1h-' in ticker_lower_check:
                return web.json_response({
                    "error": "AI analysis not available for Up/Down markets (prices change too quickly)"
                }, status=400)

            # Check cache first (saves FactsAI credits)
            cache_key = f"research:kalshi:{ticker}"
            cached = self.research_cache.get(cache_key)
            if cached:
                logger.info(f"[Kalshi Research] Cache hit for: {ticker[:50]}")
                return web.json_response(cached)
...
Keep response concise (2-3 sentences max). Use lowercase. Be friendly but professional."""
                ...
                raw_keywords = classification.get("keywords", [question])

                # Filter out common/generic words that match everything
                stopwords = {"market", "markets", "prediction", "predictions", "top", "best", "what",
                            "are", "the", "is", "a", "an", "and", "or", "for", "in", "on", "to",
                            "about", "trending", "hot", "popular", "current", "latest", "find", "show"}
                keywords = [kw for kw in raw_keywords if kw.lower() not in stopwords and len(kw) > 2]

                # If all keywords were filtered out, use the original question words
                if not keywords:
                    keywords = [w for w in question.split() if w.lower() not in stopwords and len(w) > 2][:3]

                search_term = " ".join(keywords) if keywords else question
                logger.info(f"[Research] Filtered keywords: {keywords}")

                # Block updown queries - they waste FactsAI credits
                search_lower = search_term.lower()
                if 'updown' in search_lower or 'up/down' in search_lower or 'up or down' in search_lower:
                    return web.json_response({
                        "success": True,
                        "markets": [],
                        "analysis": None,
                        "answer": "i don't analyze up/down markets - their prices change every few minutes so the analysis would be outdated by the time you read it. try asking about longer-term markets instead!"
                    })

                # Check cache first (saves FactsAI credits)
                cache_key = f"research:ask:{search_term[:100]}"
                cached = self.research_cache.get(cache_key)
                if cached:
                    logger.info(f"[Research] Cache hit for: {search_term[:50]}")
                    return web.json_response(cached)

                # FactsAI ПОЛНОСТЬЮ ОТКЛЮЧЕН для research - юзают боты/спамеры
                # Только Xiaomi (бесплатный) для чата
                facts_research = ""
                ...
                try:
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            # If no filtered markets found, get top by volume as fallback
                            if not sorted_markets:
                                logger.info(f"[Research] No keyword matches, showing top volume markets")
                                all_active = []
                                for event in all_events:
                                    for m in event.get("markets", []):
...
If no relevant markets found, explain what you know from research and suggest related topics to explore."""
                    ...
                    try:
                        async with session.post(
                            ...
                            if resp.status == 200:
                                ...
                                error_text = await resp.text()
                                logger.error(f"[Research] Xiaomi API error {resp.status}: {error_text[:300]}")
                    except Exception as e:
                        logger.error(f"[Research] Final answer error: {e}")

                # Fallback analysis from market data if AI failed
                if not analysis and markets:
                    market_lines = []
                    for m in markets[:3]:
                        q = m.get('question', '')[:50]
                        outcomes = m.get('outcomes', [])
                        prices = m.get('prices', [])
                        if outcomes and prices:
                            top_price = float(prices[0]) * 100
                            market_lines.append(f"• {q}... ({top_price:.0f}% yes)")
                    analysis = "here are the top markets:\n\n" + "\n".join(market_lines) + "\n\nask about a specific topic for deeper analysis."

                # Cache the result (1 hour TTL)
                response_data = {
                    "success": True,
                    "markets": markets[:5] if markets else [],
                    "analysis": analysis.strip() if analysis else "no markets found for this query. try asking about a different topic.",
    ...
    async def projects_confirm(self, request):
        ...
        try:
            ...
            launch["status"] = "failed"
            error_msg = str(e)
            logger.error(f"Token creation failed with exception: {error_msg}")
            return web.json_response({"success": False, "message": f"Token creation failed: {error_msg}"}, status=500)
        ...
        if result:
            ...
            # SOL remains in token wallet - we have the private keys saved
